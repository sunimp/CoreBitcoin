...
- (NSError*) scriptErrorOpcodeRequiresItemsOnStack:(NSUInteger)items {
    if (items == 1) {
        return [NSError errorWithDomain:BTCErrorDomain
                                   code:BTCErrorScriptError
                               userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:NSLocalizedString(@"%@ requires %zd item on stack.", @""), BTCNameForOpcode(_opcode), items]}];
    }
    return [NSError errorWithDomain:BTCErrorDomain code:BTCErrorScriptError userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:NSLocalizedString(@"%@ requires %zd items on stack.", @""), BTCNameForOpcode(_opcode), items]}];
}
